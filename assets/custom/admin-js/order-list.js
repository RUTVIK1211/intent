$(function() {
    ...
    $(document).delegate('#order_detail_btn', 'click', function() {
        ...
        $.ajax({
            ...
            data: { order_id: order_id, ord_type: ord_type, button_type: 'order_detail' },
    ...
    $(document).delegate('#order_tracking_btn', 'click', function() {
        ...
        $.ajax({
            type: "POST",
            url: hiddenurl + "Order_details/all_order_details",
            data: { order_id: order_id, ord_type: ord_type, button_type: 'order_tracking' },
            success: function(response) {
                $("#myModal").modal('show');
                $(".modal-body").html(response);
    ...
    $(document).delegate('#order_ndr_comment,#pre_order_ndr_comment', 'click', function() {
        ...
        if (order_id) {
            $.ajax({
                ...
                data: { order_id: order_id, order_type: order_type, other_tabs: other_tabs },
                success: function(response) {
                    if (response) {
        ...
        if (check_array != "") {
            if ($("input[name=radio_btn]:checked").length > 0) {
                ...
                $.ajax({
                    ...
                    data: { id: check_array, label_type: label_type },
                    success: function(response) {
                        if (response) {
                            $("#loader").fadeOut("slow");
